...
export async function PATCH(
  ...
  switch (action) {
    ...
    case "run": {
      ...
      await db.update(schema.agents).set({ status: "running" }).where(eq(schema.agents.id, id));
    ...
    case "pause": {
      await db.update(schema.agents).set({ status: "paused" }).where(eq(schema.agents.id, id));
      await db.insert(schema.agentActivity).values({
        id: randomUUID(), userId: user.id, agentId: id, agentName: agent.name,
    ...
    case "kill": {
      await db.update(schema.agents).set({ status: "killed" }).where(eq(schema.agents.id, id));
      await db.insert(schema.agentActivity).values({
        id: randomUUID(), userId: user.id, agentId: id, agentName: agent.name,
    ...
    case "revive": {
      await db.update(schema.agents).set({ status: "draft", backtest: null }).where(eq(schema.agents.id, id));
      return NextResponse.json({ ok: true });
    }
    default:
...
export async function DELETE(
  ...
  await db.delete(schema.agentActivity).where(eq(schema.agentActivity.agentId, id));
